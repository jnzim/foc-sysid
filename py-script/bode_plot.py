...
# High-coherence band for extraction — restrict to <500Hz, coh >= 0.8
fit_band = (f_csd >= 1.0) & (f_csd <= 500.0) & (Cxy >= 0.8)

# -----------------------------------------------------------------------------
# -3dB extraction
# -----------------------------------------------------------------------------
mag_dc  = np.mean(mag[fit_band][5:20])
mag_3db = mag_dc - 3.0
idx_3db = np.argmin(np.abs(mag[fit_band] - mag_3db))
fc_meas = f_csd[fit_band][idx_3db]
R_meas  = 1.0 / (10 ** (mag_dc / 20))
L_meas  = R_meas / (2 * np.pi * fc_meas)

print(f"\n-3dB extraction (coh>=0.8, f<=500Hz):")
print(f"  mag_dc  = {mag_dc:.2f} dB")
...
try:
    popt, pcov = curve_fit(rl_magnitude,
                           f_csd[fit_band], mag[fit_band],
                           p0=[R_NOM, L_NOM])
    R_fit, L_fit = popt
    perr   = np.sqrt(np.diag(pcov))
    fc_fit = R_fit / (2 * np.pi * L_fit)
    print(f"\nCurve fit (coh>=0.8, f<=500Hz):")
